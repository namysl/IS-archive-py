class Peak(object):
    ...
    def dac(self, L): # DIVIDE AND CONQUER
        """DO ZROBIENIA W DAC:
        - wywala się, jeśli zostają dwa elementy na rząd!
        - docstring
        ...
        for row in range(0, self._n):
            if L[row][middle] > max:
                ...
                local_loc = (row, middle) # lokalne polozenie wartosci
                x = row

        print("Największa w kolumnie:", max, "zlokalizowana w:", local_loc)

        if L[x][middle - 1] < max and max > L[x][middle + 1]: # WIERZCHOLEK
            print("Znaleziono wierzchołek:")
