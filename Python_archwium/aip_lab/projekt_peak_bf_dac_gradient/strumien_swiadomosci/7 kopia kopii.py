class Peak(object):
    ...
    def b(self, start = (0,0)):
        def gora(loca):
            x = self.get_val((globloc[0] - 1, globloc[1]))
            return x
        def dol(loca):
            xx = self.get_val((globloc[0]+ 1, globloc[1]))
            return xx
        def lewy(loca):
            xxx = self.get_val((globloc[0], globloc[1] - 1))
            return xxx
        def prawy(loca):
            xxxx = self.get_val((globloc[0], globloc[1] + 1))
            return xxxx

        def recur(loc):
            print('REWKURWWWWWWWWWW')
        ...
        globloc = self.get_loc(mniej)
        print('mniej loc glob:', globloc)

        if n == 0 and m == 0: # GÓRNY LEWY
            print('1')
            if mniej > dol(globloc) and mniej > prawy(globloc):
                print('OK! WIERZCHOLEK! a:', mniej)
            else:
                print('a')
                recur(mniejloc)
        elif n == self._n - 1 and m == 0: # DOLNY LEWY
            print('2')
            if mniej > gora(globloc) and mniej > prawy(globloc):
                print('OK! WIERZCHOLEK! b:', mniej)
            else:
                print('b')
                recur(mniejloc)
        elif n == 0 and m == self._m - 1: # GÓRNY PRAWY
            print('3')
            if mniej > dol(globloc) and mniej > lewy(globloc):
                print('OK! WIERZCHOLEK! c:', mniej)
            else:
                print('c')
                recur(mniejloc)
        elif n == self._n - 1 and m == self._m - 1: # DOLNY PRAWY
            print('4')
            if mniej > lewy(globloc) and mniej > gora(globloc):
                print('OK! WIERZCHOLEK! d:', mniej)
            else:
                print('d')
                recur(mniejloc)
        elif n == 0 and m < self._m: # górny rząd
            print('5')
            if mniej > lewy(globloc) and mniej > prawy(globloc) and mniej > dol(globloc):
                print('OK! WIERZCHOLEK! e:', mniej)
            else:
                print('e')
                recur(mniejloc)
        elif n < self._n and m == 0: # lewa kolumna
            print('6')
            if mniej > prawy(globloc) and mniej > gora(globloc) and mniej > dol(globloc):
                print('OK! WIERZCHOLEK! f:', mniej)
            else:
                print('f')
                recur(mniejloc)
        elif n == self._n - 1 and m == self._m - 1: # dolny rząd 
            print('7')
            if mniej > gora(globloc) and mniej > lewy(globloc) and mniej > prawy(globloc):
                print('OK! WIERZCHOLEK! g:', mniej)
            else:
                print('g')
                recur(mniejloc)
        elif n < self._n and m == self._m - 1: # prawa kolumna 
            print('8')
            if mniej > lewy(globloc) and mniej > gora(globloc) and mniej > dol(globloc):
                print('OK! WIERZCHOLEK! h:', mniej)
            else:
                print('h')
                recur(mniejloc)
            ...
            print('gora:', gora(globloc), 'dol:', dol(globloc), 'lewy:', lewy(globloc), 'prawy:', prawy(globloc))
        
            if mniej > gora(globloc) and mniej > dol(globloc) and mniej > lewy(globloc) and mniej > prawy(globloc):
                print('OK! WIERZCHOLEK!:', mniej)
            else:
                recur(self.get_loc(mniej))
