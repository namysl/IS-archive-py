class Peak(object):
    ...
    def gaa(self, start = (0,0)): # GREEDY ASCENT Algorithm
        def go_up(loc):
            up = start_loc[0] - 1, start_loc[1] ; return up
        def go_down(loc):
            down = start_loc[0]+ 1, start_loc[1] ; return down
        def go_left(loc):
            left = start_loc[0], start_loc[1] - 1 ; return left
        def go_right(loc):
            right = start_loc[0], start_loc[1] + 1 ; return right
        ...
        while not found:
            ...
            n = start[0]
            m = start[1]
            start_val = self.get_val(start_loc)
            
            if start_val < self.get_val(go_right(start_loc)) and m < self._m:
                print('1 if! start_loc:', start_loc, 'start_val:', start_val)
                start_val = go_right(start_loc)
            else:
                next_el_loc = go_down(start_loc)
                print('1 else! start_loc:', start_loc, 'start_val:', start_val, 'next_el_loc:', next_el_loc, 'next_el_val', next_el_val)
                if start_val < self.get_val((go_down(start_loc))) and m < self._m:
                    print('2 if! start_loc:', start_loc, 'start_val:', start_val, 'next_el_loc:', next_el_loc, 'next_el_val', next_el_val)
                    next_el_loc = go_down(start_loc)
