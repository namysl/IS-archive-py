...
void PokerTest(uint32_t* tab,uint32_t n,uint32_t bits,uint32_t shift){
    ...
    for (int i=0; i < m; i++){
       sum += (licznik[i]-((double)(n/m)))*(licznik[i]-((double)(n/m)));
    }

    result = (double)(sum / (double)(n/m));

    if ((chi2d[bits-1] < result) && (result < chi2u[bits-1])){
       printf(" Tak %d, %u \n", shift, shift+bits);
    } else {
       printf(" Nie %d, %u \n", shift, shift+bits);
    }
