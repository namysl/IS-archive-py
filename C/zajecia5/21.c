...
void PokerTest(unsigned* tab,unsigned n,unsigned bits,unsigned shift){
    //tab -> tablica liczb 32bitowych, rozmiar tablicy to: 2^bits
    //n -> rozmiar, n liczb o m bitach
    //sprawdzamy bity od shift do shift+bits-1

    unsigned dwadobits = 1<<bits; //2^0 = 1 przesuwamy o bits
    long long licznik[dwadobits];

    //zerujemy liczniki
    //memset(licznik, 0, dwadobits-1);

    for (int i=0; i<dwadobits; i++){
        licznik[i]=0;
    }
    ...
    for(int i=0; i<n; i++){
        // 001010010101
        // --      ----
        licznik[ (tab[i] >> shift) & (dwadobits-1) ]++;
    }

    double x, y, suma = 0;
    y = n;
    y /= dwadobits;  // to jest n/(2^m)
    ...
    if(suma<chi2u[bits-1] && suma>chi2d[bits-1]){
        printf("zaliczony, ");
    }
    else{
        printf("niezaliczony, ");
    ...
    printf("bity= %2u - %2u, suma= %7.4f (%7.4f, %7.4f)\n",
           shift, (bits+shift-1), suma, chi2d[bits-1], chi2u[bits-1]);
}
